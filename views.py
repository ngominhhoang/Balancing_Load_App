...
def run_process(id,level="1",percentage="30"):
    compiler_name = id+"_Balancing_Load"
    cppfile_name = os.path.join(app.config["FILE_RESULTS"],"Balancing_Load.cpp")
    process_name = os.path.join(app.config["FILE_RESULTS"],compiler_name)
    if (os.system('g++ -o ' + process_name +' ' + cppfile_name) == 0):
